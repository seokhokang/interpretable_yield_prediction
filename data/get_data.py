...
if __name__ == "__main__":
    ...
    for data_id in [1, 2]:
        for split_id in range(10):
            load_dict = np.load('./split/dataset_%d_%d.npz'%(data_id, split_id))
            
            rsmi_list = load_dict['rxn']
            yld_list = load_dict['yld']
            
            filename = './molgraph_dataset_%d_%d.npz'%(data_id, split_id)
            print(filename)            
            get_graph_data(rsmi_list, yld_list, filename)

    for test_id in [1, 2, 3, 4]:
        load_dict = np.load('./split/test_%d.npz'%test_id)

        rsmi_list = load_dict['rxn']
        yld_list = load_dict['yld']

        filename = './molgraph_test_%d.npz'%test_id
        print(filename)
        get_graph_data(rsmi_list, yld_list, filename)