...
def training(net, cuda, train_loader, val_loader, train_y_mean, train_y_std, val_monitor_epoch = 10, n_forward_pass = 5):
    ...
    for epoch in range(n_epochs):
        ...
        #print(torch.triu(torch.cov(out[:,:,0].permute(1,0)), diagonal = 1).square().sum(), torch.triu(torch.corrcoef(out[:,:,0].permute(1,0)), diagonal = 1).square().sum())
        #print(torch.cov(out[:,:,0].permute(1,0)))
        #print(torch.corrcoef(out[:,:,0].permute(1,0)))
        #print(loss_a.item(), loss_b.item(), loss_c.item())
